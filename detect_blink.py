...
# -----------------------------
# Blink detection via EAR using MediaPipe Face Mesh
# -----------------------------

mp_face_mesh = mp.solutions.face_mesh

# FaceMesh settings:
# refine_landmarks=True enables iris landmarks and generally improves eye landmark quality
...
# MediaPipe FaceMesh landmark indices for eyes (commonly used)
# We'll compute EAR using 6 points for each eye:
...
def normalized_to_pixel_coords(landmarks, w, h):
    """
    Convert MediaPipe normalized landmarks to pixel coords as Nx2 array.
    """
...
def main():
    ...
    # ---- Tunable parameters ----
    EAR_THRESHOLD = 0.21        # lower => stricter. Typical range ~0.18-0.25 depending on camera/face size
    CONSEC_FRAMES = 2           # how many consecutive frames below threshold to count as a blink
    SHOW_DEBUG = True
    ...
    while True:
        ...
        if SHOW_DEBUG:
            if ear is not None:
                cv2.putText(frame, f"EAR: {ear:.3f}  (thr={EAR_THRESHOLD:.2f})", (20, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, f"FPS: {fps:.1f}", (20, 120),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

        cv2.imshow("MediaPipe Blink Detector (press q to quit)", frame)
