...
# # GIVES direction, doesn't reach ends:
# DOT_RADIUS = 10
# GRID_ROWS_Y = 24
# GRID_COLS_Y = 3
# DOT_PAUSE = 1  # seconds to pause on each dot
# FRAMES_PER_POINT = 2
...
#     nose = lm_to_pixel(landmarks[NOSE_TIP])
...
#     grid = [(int(x), int(y))  for x in xs for y in ys]
#     return grid
...
SCREEN_WIDTH, SCREEN_HEIGHT = pyautogui.size()
DOT_RADIUS = 15
GRID_ROWS_Y = 9  # Increased for better coverage
GRID_COLS_Y = 5  # Increased for better coverage
DOT_PAUSE = 1.5  # Slightly longer
WARMUP_FRAMES = 10  # Skip initial unstable frames
...
def get_eye_features(landmarks, image_shape):
    ...
    return [left_ratio_y, right_ratio_y, avg_ratio_y, left_height, right_height, avg_height, normalized_y]
...
try:
    for idx, point in enumerate(grid_points_y):
        x, y = point
        print(f"Point {idx + 1}/{len(grid_points_y)}: {point}")
        ...
        while time.time() - start_time < DOT_PAUSE:
            ...
            if results.multi_face_landmarks:
                ...
                # Skip warmup frames for stability
                if frame_count > WARMUP_FRAMES:
                    point_samples.append(features + [x, y])

            # Display
            display_frame = np.zeros((SCREEN_HEIGHT, SCREEN_WIDTH, 3), dtype=np.uint8)
            cv2.circle(display_frame, (x, y), DOT_RADIUS, (0, 255, 255), -1)
        ...
        dataset_y.extend(point_samples)
...
columns = [
    "left_ratio_y", "right_ratio_y", "avg_ratio_y",
    "left_height", "right_height", "avg_height", "normalized_y",
    "screen_x", "screen_y"
]
df = pd.DataFrame(dataset_y, columns=columns)
df.to_csv("eye_calibration.csv", index=False)
