...
# # Helper to get eye features (only x for now)
...
#     left_ratio_y = (left_iris[1] - left_inner[1]) / left_width  
...
#     right_ratio_y = (right_iris[1] - right_inner[1]) / right_width  
...
# try:
#     # last_y = SCREEN_HEIGHT // 2  # fixed Y for now
#     while True:
#         ret, frame = cap.read()
#         if not ret:
#             continue
...
#             x_coord = predict_x(features_x)

#             features_y = [features[2], features[3]]
#             y_coord = predict_y(features_y)

#             # Draw dot
#             display_frame = np.zeros((SCREEN_HEIGHT, SCREEN_WIDTH, 3), dtype=np.uint8)
#             cv2.circle(display_frame, (int(x_coord), y_coord), 15, (0, 255, 0), -1)
