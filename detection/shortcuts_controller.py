...
CONFIG_PATH = ROOT_DIR / "desktop-pet" / "config" / "gesture_map.json"
STT_SCRIPT = ROOT_DIR / "Speech_to_text" / "stream_stt.py"
...
def start_stt(stt_proc):
    ...
    try:
        if not STT_SCRIPT.exists():
            return stt_proc
        return subprocess.Popen([sys.executable, "-u", str(STT_SCRIPT)])
    except Exception:
        return stt_proc
...
def main():
    ...
    def _handle_open_buddy():
        app_path = ROOT_DIR / "desktop-pet" / "config_app.py"
        if app_path.exists():
            try:
