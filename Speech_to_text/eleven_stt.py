...
def normalize_intent(text):
    """
    Normalize text to extract the core intent/action.
    Removes filler words and normalizes spacing.
    """
    text = text.lower().strip()
    # Remove common filler words/phrases
    fillers = ["hey buddy", "hey", "buddy", "please", "can you", "could you", 
               "uh", "um", "like", "so", "just", "i think", "you know"]
        ...
        try:
            ...
            async with websockets.connect(
                ...
                state = {
                    ...
                    "had_text": False,
                    "last_committed": None,
                    "last_intent": None,  # Track the last normalized intent
                }
                        ...
                        if msg_type in ("partial_transcript", "committed_transcript"):
                            ...
                            if text:
                                ...
                                if msg_type == "partial_transcript":
                                    ...
                                    # Normalize the intent and check if it's different from the last one
                                    current_intent = normalize_intent(text)
                                    if current_intent != state["last_intent"]:
                                        state["last_intent"] = current_intent
                                        # YOUR ACTION CODE GOES HERE
                                        print(f"✨ EXECUTING ACTION: {current_intent}", flush=True)
                                    else:
                                        print(f"⏭️ Skipping duplicate intent: {current_intent}", flush=True)
                            continue
                        # Surface any errors or unexpected messages
                        if msg_type and "error" in msg_type:
                            print(f"❌ {msg_type}: {event}", flush=True)
